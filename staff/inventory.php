...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    adjust_stock($productId, $type === 'add' ? $amount : -$amount, $type, $user['user_id'], 'Adjusted by staff');
    set_flash('success', 'Stock updated.');
    redirect(BASE_URL . 'staff/inventory.php');
}
...
$stmt = $pdo->prepare("SELECT p.product_id, p.product_name, i.quantity, i.stock_status FROM inventory i JOIN products p ON p.product_id = i.product_id WHERE $where ORDER BY i.quantity ASC");
$stmt->execute($params);
$rows = $stmt->fetchAll();
  ...
  <main class="sc-content">
    ...
    <div class="sc-table-wrap">
      <table class="table align-middle mb-0">
        <thead class="sc-thead"><tr><th>Product</th><th>Stock</th><th>Status</th><th>Quick Adjust</th></tr></thead>
        <tbody>
