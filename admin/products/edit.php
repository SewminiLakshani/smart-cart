...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['update_product'])) {
        ...
        // Image upload problems are shown as warnings but never block saving the rest of the product's fields.
        $imageErrors = [];
        $newImagePaths = [];
        if (!empty($_FILES['images']['name'][0])) {
            foreach ($_FILES['images']['name'] as $i => $name_) {
                if ($_FILES['images']['error'][$i] !== UPLOAD_ERR_OK) { $imageErrors[] = upload_error_message($_FILES['images']['error'][$i]); continue; }
                ...
                catch (RuntimeException $ex) { $imageErrors[] = $ex->getMessage(); }
            }
        }
        ...
        if (empty($errors)) {
            ...
            if (!empty($imageErrors)) {
                set_flash('warning', 'Product details were saved, but: ' . implode(' ', $imageErrors));
            } else {
